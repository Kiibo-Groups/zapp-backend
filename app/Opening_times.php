...
class Opening_times extends Authenticatable
{
    ...
    public function ViewNextTime($id) // Store Closed - Next Open
    {
        if ($this->GetTimes($id)) {
            $times = $this->GetTimes($id);
            ...
            $viewTimes = $this->ViewTimeDate($times,$day);
            return $viewTimes['open_t_app'].' - '.$viewTimes['close_t_app'];
        }
    }
    ...
    public function ViewTimeDate($data,$day)
    {
        ...
        $mDate = new DateTime($open_time);
        $cDate = new DateTime($close_time);

        return [
            'date' => $date, 
            'open_time' => $open_time, 
            'close_time' => $close_time,
            'open_t_app' => $mDate->format("g:i A"),
            'close_t_app' => $cDate->format("g:i A") 
